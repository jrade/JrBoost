...
TreeTrainer<SampleIndex>::TreeTrainer(CRefXXfc inData, CRefXu8 strata) :
    inData_{inData}, sampleCount_{static_cast<size_t>(inData.rows())},
    variableCount_{static_cast<size_t>(inData.cols())}, sortedSamples_{initSortedSamples_()}, strata_{strata},
...
template<typename SampleIndex>
vector<vector<SampleIndex>> TreeTrainer<SampleIndex>::initSortedSamples_() const
{
...
template<typename SampleIndex>
vector<size_t> TreeTrainer<SampleIndex>::initSampleCountByStratum_() const
{
...
TreeTrainer<SampleIndex>::train(CRefXd outData, CRefXd weights, const BaseOptions& options, size_t threadCount) const
{
    ...
    if (maxStatusCount <= (1 << 8))
        pred = trainImpl_<uint8_t>(outData, weights, options, threadCount);
    else if (maxStatusCount <= (1 << 16))
        pred = trainImpl_<uint16_t>(outData, weights, options, threadCount);
    else if (maxStatusCount <= (1LL << 32))
        pred = trainImpl_<uint32_t>(outData, weights, options, threadCount);
    else
        pred = trainImpl_<uint64_t>(outData, weights, options, threadCount);
...
unique_ptr<const BasePredictor> TreeTrainer<SampleIndex>::trainImpl_(
...
#pragma omp parallel num_threads(static_cast <int>(threadCount)) private(ITEM_COUNT)
    ...
    /*
    template<typename SampleIndex>
    void TreeTrainer<SampleIndex>::validateData_(CRefXd outData, CRefXd weights) const
    {
...
template<typename SampleIndex>
void TreeTrainer<SampleIndex>::initWyPacks(CRefXd outData, CRefXd weights) const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
...
template<typename SampleIndex>
size_t TreeTrainer<SampleIndex>::usedVariableCount_(const BaseOptions& options) const
{
...
template<typename SampleIndex>
size_t TreeTrainer<SampleIndex>::initUsedVariables_(const BaseOptions& options) const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
    ThreadLocalData1_<SampleIndex>& t1 = threadLocalData1_<SampleIndex>;
    ...
    if (!options.saveMemory() && !options.selectVariablesByLevel() && options.maxTreeDepth() != 1)
...
template<typename SampleIndex>
void TreeTrainer<SampleIndex>::initTree_() const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
...
template<typename SampleIndex>
template<typename SampleStatus>
size_t TreeTrainer<SampleIndex>::initSampleStatus_(CRefXd outData, CRefXd weights, const BaseOptions& options) const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
    ThreadLocalData1_<SampleIndex>& t1 = threadLocalData1_<SampleIndex>;
    ThreadLocalData2_<SampleStatus>& t2 = threadLocalData2_<SampleStatus>;
    ...
    const double minSampleWeight
        = (options.minRelSampleWeight() == 0)
              ? options.minAbsSampleWeight()
              : std::max(options.minAbsSampleWeight(), options.minRelSampleWeight() * weights.maxCoeff());
    ...
    if (minSampleWeight == 0.0) {
        ...
        if (options.usedSampleRatio() == 1.0) {
        ...
        else if (!options.stratifiedSamples()) {
            ...
            for (size_t z = 0; z != stratumCount_; ++z) {
                ...
                m[z] = static_cast<size_t>(std::round(options.usedSampleRatio() * n[z]));
            }
...
template<typename SampleIndex>
template<typename SampleStatus>
void TreeTrainer<SampleIndex>::updateSampleStatus_(CRefXd outData, CRefXd weights, size_t d) const
{
...
template<typename SampleIndex>
template<typename SampleStatus>
const SampleIndex* TreeTrainer<SampleIndex>::initOrderedSamples_(
    size_t usedVariableIndex, size_t usedSampleCount, const BaseOptions& options, size_t d) const
{
    ...
    if (d + 1 != options.maxTreeDepth() && !options.saveMemory() && !options.selectVariablesByLevel())
...
template<typename SampleIndex>
template<typename SampleStatus>
const SampleIndex* TreeTrainer<SampleIndex>::updateOrderedSampleSaveMemory_(
    size_t usedVariableIndex, size_t usedSampleCount, const BaseOptions& /*options*/, size_t d) const
{
    // called from the inner threads so be careful to distinguish between t0 and t0.parent etc.
...
template<typename SampleIndex>
template<typename SampleStatus>
const SampleIndex* TreeTrainer<SampleIndex>::updateOrderedSamples_(
    size_t usedVariableIndex, size_t usedSampleCount, const BaseOptions& options, size_t d) const
{
...
template<typename SampleIndex>
void TreeTrainer<SampleIndex>::initNodeTrainers_(const BaseOptions& options, size_t d, size_t threadCount) const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
    ThreadLocalData1_<SampleIndex>& t1 = threadLocalData1_<SampleIndex>;
...
template<typename SampleIndex>
void TreeTrainer<SampleIndex>::updateNodeTrainers_(
#if !PACKED_DATA
...
template<typename SampleIndex>
size_t TreeTrainer<SampleIndex>::finalizeNodeTrainers_(size_t d, size_t threadCount) const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
    ThreadLocalData1_<SampleIndex>& t1 = threadLocalData1_<SampleIndex>;
...
template<typename SampleIndex>
unique_ptr<const BasePredictor> TreeTrainer<SampleIndex>::createPredictor_() const
{
    ThreadLocalData0_& t0 = threadLocalData0_;
...
template class TreeTrainer<uint8_t>;
template class TreeTrainer<uint16_t>;
template class TreeTrainer<uint32_t>;
template class TreeTrainer<uint64_t>;
